<template>
  ...
  <!-- Stable Coin -->
  <div v-if="transcations || totalSupply || topKHolder || tokenPrice || mintBurn || holdAgeFreqPortfilio">
    <div class="d-flex align-items-center" style="cursor: pointer;">
      <h2>Stable Coin</h2>
      <div class="ms-auto">Last Update {{ formatDateTime(new Date()) }}</div>
    </div>
    ...
    <div class="row row-deck row-cards mb-3">
      ...
      <div v-if="transcations" class="col-lg-6">
        <ChartComponent type="bar" title="Daily Frequency"
          :labels="[transcations.daily_frequency.data.map((d: any) => `${d[0]}`)]" :data="[
            {
      ...
      <div v-if="transcations" class="col-lg-6">
        <ChartComponent type="bar" title="Hourly Frequency"
          :labels="[transcations.hourly_frequency.data.map((d: any) => `${d[0]}:00`)]" :data="[
            {
    ...
    <div class="row row-deck row-cards mb-3">
      ...
      <div v-if="topKHolder" class="col-lg-6">
        <PieChartComponent title="Top 10 holders" :labels="topKHolder.data.map((d: any) => String(d[0]).substring(0, 10))"
          :data="topKHolder.data.map((d: any) => d[3])" :update-at="updateAt" />
      </div>
      <div v-if="holdAgeFreqPortfilio" class="col-lg-6">
        <PieChartComponent title="Age Stats" :labels="holdAgeFreqPortfilio['age'].data.map((d: any) => String(d[1]))"
          :data="holdAgeFreqPortfilio['age'].data.map((d: any) => Number(d[2]))" :update-at="updateAt" />
      </div>
      <div v-if="holdAgeFreqPortfilio" class="col-lg-6">
        <PieChartComponent title="Freq Stats" :labels="holdAgeFreqPortfilio['freq'].data.map((d: any) => String(d[1]))"
          :data="holdAgeFreqPortfilio['freq'].data.map((d: any) => Number(d[2]))" :update-at="updateAt" />
      </div>
      <div v-if="holdAgeFreqPortfilio" class="col-lg-6">
        <PieChartComponent title="Portfolio Stats"
          :labels="holdAgeFreqPortfilio['portfolio'].data.map((d: any) => String(d[1]))"
          :data="holdAgeFreqPortfilio['portfolio'].data.map((d: any) => Number(d[2]))" :update-at="updateAt" />
      </div>
    </div>
  </div>
